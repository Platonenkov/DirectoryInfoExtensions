...
using DirectoryInfoExtensions;
using DirectoryInfoExtensions.Trees;
...
namespace System.IO
{
    ...
    public static class DirectoryInfoExtensions
    {
        ...
        public static FileInfo Zip(this DirectoryInfo Directory, FileInfo ArchiveFile)
        {
            ...
            const int buffer_size = DataLength.Bytes.MB;
        ...
        public static Process ShowInFileExplorer(this FileSystemInfo dir) => Process.Start("explorer", $"/select,\"{dir.FullName}\"") ?? throw new InvalidOperationException();
        ...
        /// <summary>Определить объем всех вложенных файлов включая поддиректории</summary>
        /// <param name="Directory">Исследуемая директория</param>
        ...
        public static IEnumerable<FileInfo> FindFiles(this DirectoryInfo dir, string mask) => dir.EnumerateDirectories().SelectMany(d => d.FindFiles(mask)).InsertBefore(dir.EnumerateFiles(mask));

        #endregion

        #region Sub | Positions
        ...
        /// <summary>Формирование информации о поддиректории, заданной своим именем, либо относительным путем</summary>
        ...
        internal static void CopyTo(this Stream input, Stream output, byte[] Buffer)
        {
        ...
        [DST]
        internal static void Foreach<TArray>(
            this TArray[] array,
        ...
        /// <returns>Результирующая последовательность элементов, в которой добавленный элемент идет на первом месте</returns>
        internal static IEnumerable<T> InsertBefore<T>(this IEnumerable<T> enumerable, IEnumerable<T> values)
        {
        ...
        internal static TResult? Try<T, TResult>(
